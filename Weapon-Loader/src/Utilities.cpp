...
void* GetProcAddressH(HMODULE ModuleBase, std::uint32_t FunctionHash) {
	...
	if (ModuleBase == nullptr) {
		std::cerr << "{!!} GetProcAddressH: null module pointer." << std::endl;
		return nullptr;
	}
	...
	std::cerr << "{!!} GetProcAddressH: failed to get function. Hash:" << FunctionHash << std::endl;
	return nullptr;
}
...
byte* GetModuleHandleH(std::uint32_t ModuleHash) {
	...
	std::cerr << "{!!} GetModuleHandleH: failed to retrieve function pointer. Hash: " << ModuleHash << std::endl;
	return nullptr;
}
...
void Decrypt(byte* PayloadBuffer, size_t payloadSize) { //extremely rudimentary encryption via XOR
